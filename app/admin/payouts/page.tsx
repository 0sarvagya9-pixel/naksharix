...
export default async function AdminPayoutsPage() {
  ...
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-200">Finance Ops</p>
          ...
          {pendingRows.map(({ profile, pending, gross, paid }) => (
            <Card key={profile.id} className="glass">
              <CardContent className="grid gap-2 p-5 md:grid-cols-4">
                ...
                <p className="text-sm text-muted-foreground">Gross INR {gross.toLocaleString("en-IN")}</p>
                <p className="text-sm text-muted-foreground">Paid INR {paid.toLocaleString("en-IN")}</p>
                <p className="text-sm text-amber-200">Pending INR {pending.toLocaleString("en-IN")}</p>
              </CardContent>
            </Card>
          ))}
          {!pendingRows.length ? <p className="rounded-lg border border-amber-200/15 bg-card/60 p-4 text-sm text-muted-foreground">No pending payout balances found.</p> : null}
        </div>
          ...
          {payouts.map((payout) => (
            <Card key={payout.id} className="border-amber-200/15 bg-card/70">
              <CardContent className="grid gap-2 p-5 md:grid-cols-4">
                ...
                <p className="text-sm text-muted-foreground">INR {Number(payout.amount).toLocaleString("en-IN")}</p>
                <p className="text-sm text-muted-foreground">{payout.status}</p>
                <p className="text-sm text-muted-foreground">{payout.createdAt.toDateString()}</p>
              </CardContent>
            </Card>
          ))}
        </div>
