...
export default async function AstrologerEarningsPage() {
  ...
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#FFD700]">Earnings</p>
        ...
        <div className="mt-8 grid gap-4">
          ...
          {!profile?.payouts.length ? <p className="rounded-lg border border-[#D4AF37]/20 bg-[#061D3C]/70 p-4 text-sm naksh-muted-text">No payouts recorded yet.</p> : null}
        </div>
      </Section>
    </main>
  );
  ...
  return (
    <Card className="glass">
      <CardContent className="p-5">
        {label.includes("Pending") ? <Wallet className="h-5 w-5 text-[#FFD700]" /> : <IndianRupee className="h-5 w-5 text-[#FFD700]" />}
