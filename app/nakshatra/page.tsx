...
export default function NakshatraIndexPage() {
  return (
    <main className="star-field">
      <Section>
        <h1 className="font-cinzel text-4xl font-black text-[#1F2933]">27 Nakshatra Pages</h1>
        <p className="mt-4 max-w-3xl naksh-muted-text">Explore lunar mansion meanings and connect them with Kundli, horoscope, and reviewed report workflows.</p>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {nakshatras.map((slug) => (
            <Link key={slug} href={`/nakshatra/${slug}`}>
              <Card className="h-full border-[#E7D8BE] bg-white/88 transition hover:border-[#D8AF66]/55">
                <CardContent className="p-5">
                  <h2 className="font-cinzel text-xl font-bold text-[#1F2933]">{titleFromSlug(slug)}</h2>
