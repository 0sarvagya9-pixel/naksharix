      ...
      <Section first>
        ...
        <div className="glass-panel rounded-3xl p-6 md:p-10 mb-8">
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
            <div>
              <span
                className="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-[rgba(216,154,43,0.08)] border border-[rgba(216,154,43,0.18)] text-[#c98924]"
              >
                Provider Verified
              </span>
              <h1 className="mt-4 font-cinzel text-4xl font-black text-[#17181d] sm:text-5xl">
                Today&apos;s Panchang
              </h1>
              ...
              <div className="mt-6 flex flex-wrap gap-3 text-sm">
                {[
                  { icon: <MapPin className="h-4 w-4 text-[#c98924]" />,   label: "Location-aware" },
                  { icon: <Clock3 className="h-4 w-4 text-[#c98924]" />,   label: "Timezone-based" },
                  { icon: <SunMedium className="h-4 w-4 text-[#c98924]" />, label: "Sunrise anchored" },
                ].map(({ icon, label }) => (
                  <span
                    key={label}
                    className="inline-flex items-center gap-2 rounded-full px-4 py-2 border border-[rgba(255,255,255,0.50)] bg-[rgba(255,255,255,0.52)] text-[#525866] shadow-sm"
                  >
                    {icon}{label}
                  </span>
                ))}
            ...
            <Card>
              <CardContent className="p-5">
                <form className="grid gap-4" action="/panchang">
                  <label className="grid gap-2 text-sm font-semibold text-[#17181d]">
                    Date
                    <input
                      type="date"
                      name="date"
        ...
        <Card className="mt-8">
          <CardContent className="p-5">
            <div className="flex gap-3">
              ...
              <div>
                <h2 className="font-cinzel text-xl font-bold text-[#17181d]">
                  Trust note
                </h2>
...
function PanchangCard({ title, icon, rows }: { title: string; icon: ReactNode; rows: Array<[string, string | null]> }) {
  return (
    <Card className="h-full">
      <CardContent className="p-5">
        <h2 className="flex items-center gap-2 font-cinzel text-xl font-bold text-[#17181d]">
          <span className="text-[#c98924]">{icon}</span>
          {title}
        </h2>
        <div className="mt-4 grid gap-3">
          {rows.map(([label, value]) => (
            <div
              key={label}
              className="rounded-xl p-3 border border-[rgba(255,255,255,0.50)] bg-[rgba(255,255,255,0.52)] shadow-sm"
            >
              <p className="text-xs uppercase tracking-[0.16em] text-[#525866]">
                {label}
              </p>
              <p className="mt-1 text-sm font-semibold text-[#17181d]">
                {value ?? "Unavailable"}
              </p>
            </div>
          ))}
