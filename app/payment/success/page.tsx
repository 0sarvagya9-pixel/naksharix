...
export default async function PaymentSuccessPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const title = isAdminMode ? safeServerTranslation(locale, "adminAccess") : isVip ? "VIP" : "Premium";
  const thankYou = isVip ? safeServerTranslation(locale, "vipSuccessMessage") : safeServerTranslation(locale, "premiumSuccessMessage");
    ...
    <Section className="max-w-3xl">
      <Card className="glass overflow-hidden border-[#D4AF37]/30">
        <CardHeader className="border-b border-[#D4AF37]/15 bg-[radial-gradient(circle_at_top,rgba(245,190,88,0.18),transparent_24rem)] text-center">
          ...
          <CardTitle className="mt-4 font-cinzel text-3xl text-[#FFFFFF]">{safeServerTranslation(locale, "paymentSuccessful")}</CardTitle>
        ...
        <CardContent className="space-y-5 p-6 text-center">
          ...
          <div className="space-y-3 text-base leading-8 naksh-muted-text">
            ...
            <p>{safeServerTranslation(locale, "contactSupportCare")}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Button asChild>
              <Link href={payment ? `/report-request/new?orderId=${payment.id}&plan=${isVip ? "VIP" : "PREMIUM"}` : "/dashboard"}>{payment ? "Submit Report Details" : safeServerTranslation(locale, "goToDashboard")}</Link>
            </Button>
            <Button asChild variant="outline">
              <Link href="/pricing">{safeServerTranslation(locale, "backToPricing")}</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
