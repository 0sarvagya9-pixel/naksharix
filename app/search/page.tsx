...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#FFD700]">Search</p>
          ...
          {results.map(([label, href]) => (
            <Link key={href} href={href}>
              <Card className="glass h-full transition hover:border-[#D4AF37]/55">
                <CardHeader><CardTitle className="flex items-center gap-2 font-cinzel"><Search className="h-4 w-4 text-[#FFD700]" />{label}</CardTitle></CardHeader>
