...
export default function ShopPage() {
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-200">Astrology Ecommerce</p>
        <h1 className="mt-3 font-cinzel text-4xl font-black">Gemstones, Rudraksha and Yantra</h1>
        <p className="mt-4 max-w-3xl text-muted-foreground">Naksharix shop is prepared as a production placeholder. Products stay in coming-soon mode until inventory, pricing, and fulfillment are configured.</p>
          ...
          {products.map(([name, copy, status]) => (
            <Card key={name} className="glass">
              <CardContent className="p-6">
                ...
                <p className="mt-4 inline-flex rounded-full border border-amber-200/20 px-3 py-1 text-xs text-amber-200">{status}</p>
                <Button className="mt-5 w-full" disabled><Package className="h-4 w-4" /> Shop coming soon</Button>
              </CardContent>
            </Card>
          ))}
        </div>
        <Card className="mt-8 border-amber-200/15 bg-card/70">
          <CardContent className="p-6">
            <ShieldCheck className="h-6 w-6 text-amber-200" />
            <h2 className="mt-4 font-cinzel text-2xl font-black">Recommendation flow</h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">Generate a Kundli or premium report first, then use recommendations as reflective guidance. Product purchase should always include quality verification and ethical disclaimers.</p>
