...
export default function ShopPage() {
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#FFD36A]">Astrology Ecommerce</p>
          ...
          {products.map(([name, copy, status]) => (
            <Card key={name} className="glass">
              <CardContent className="p-6">
                <Gem className="h-6 w-6 text-[#FFD36A]" />
                <h2 className="mt-4 font-cinzel text-2xl font-bold">{name}</h2>
                <p className="mt-3 text-sm leading-6 naksh-muted-text">{copy}</p>
                <p className="mt-4 inline-flex rounded-full border border-[#F5C542]/25 px-3 py-1 text-xs text-[#FFD36A]">{status}</p>
                <Button className="mt-5 w-full" disabled><Package className="h-4 w-4" /> Shop coming soon</Button>
              </CardContent>
            </Card>
          ))}
        </div>
        <Card className="mt-8 border-[#F5C542]/20 bg-[#201037]/75">
          <CardContent className="p-6">
            <ShieldCheck className="h-6 w-6 text-[#FFD36A]" />
