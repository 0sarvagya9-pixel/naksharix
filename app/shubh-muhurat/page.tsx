...
export default function ShubhMuhuratPage() {
  ...
  return (
    <main className="star-field">
      <Section>
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-200">Muhurat Finder</p>
        <h1 className="mt-3 font-cinzel text-4xl font-black">Shubh Muhurat</h1>
        <p className="mt-4 max-w-3xl text-muted-foreground">Use Naksharix for auspicious timing categories and daily panchang context. For major rituals, confirm with a qualified astrologer.</p>
        ...
        <Card className="glass mt-10">
          <CardContent className="p-6">
            ...
            <p className="mt-3 text-lg text-amber-200">{panchang.muhurat}</p>
            <p className="mt-2 text-sm text-muted-foreground">Choghadiya: {panchang.choghadiya} · Nakshatra: {panchang.nakshatra} · Tithi: {panchang.tithi}</p>
          </CardContent>
        </Card>
        <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {categories.map((category) => (
            <Card key={category} className="border-amber-200/15 bg-card/70">
              <CardContent className="p-5">
                <Sparkles className="h-5 w-5 text-amber-200" />
                <h2 className="mt-4 font-cinzel text-xl font-bold">{category}</h2>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">Use location, panchang, lunar day, nakshatra, and family context before finalizing this timing.</p>
              </CardContent>
            </Card>
          ))}
        </div>
