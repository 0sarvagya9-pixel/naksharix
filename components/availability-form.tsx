...
export function AvailabilityForm({ slots = [] }: { slots?: Slot[] }) {
  ...
  return (
    <div className="space-y-6">
      <form action={addSlot} className="grid gap-4 rounded-lg border border-[#F5C542]/20 bg-[#201037]/70 p-4 md:grid-cols-6">
        <div className="space-y-2">
          <Label>Day</Label>
          <select name="dayOfWeek" data-field="dayOfWeek" defaultValue="1" className={`h-10 w-full rounded-md border border-[#F5C542]/20 bg-[#12051f] px-3 text-sm ${errorClass(Boolean(errors.dayOfWeek))}`} onChange={() => clear("dayOfWeek")}>{days.map((day, index) => <option key={day} value={index}>{day}</option>)}</select>
        </div>
        ...
        <div className="space-y-2"><Label>Type</Label><select name="consultationType" defaultValue="CHAT" className="h-10 w-full rounded-md border border-[#F5C542]/20 bg-[#12051f] px-3 text-sm"><option value="CHAT">Chat</option><option value="CALL">Call</option><option value="VIDEO">Video</option></select></div>
        <div className="space-y-2"><Label>Status</Label><select name="status" data-field="status" defaultValue="ONLINE" className={`h-10 w-full rounded-md border border-[#F5C542]/20 bg-[#12051f] px-3 text-sm ${errorClass(Boolean(errors.status))}`} onChange={() => clear("status")}><option value="ONLINE">Online</option><option value="BUSY">Busy</option><option value="OFFLINE">Offline</option></select></div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {localSlots.map((slot) => <div key={slot.id} className="rounded-lg border border-[#F5C542]/20 bg-[#201037]/70 p-4"><p className="font-cinzel font-bold">{days[slot.dayOfWeek]} | {slot.consultationType ?? "CHAT"}</p><p className="mt-1 text-sm naksh-muted-text">{slot.startTime} - {slot.endTime} | {slot.isActive === false ? "Inactive" : "Active"}</p><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" type="button" onClick={() => updateSlot(slot.id, slot.isActive === false)}><Power className="h-4 w-4" />Toggle</Button><Button size="sm" variant="outline" type="button" onClick={() => deleteSlot(slot.id)}><Trash2 className="h-4 w-4" />Delete</Button></div></div>)}
      </div>
      {!localSlots.length ? <p className="rounded-lg border border-[#F5C542]/20 bg-[#201037]/70 p-4 text-sm naksh-muted-text">No availability slots added yet.</p> : null}
    </div>
  );
}