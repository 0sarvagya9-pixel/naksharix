...
export function ConsultationRequestForm({ astrologerId, astrologerName, userEmail, userName, slots }: Props) {
  ...
  return (
    <Card className="glass">
      ...
      <CardContent>
        {!userEmail ? <div className="mb-5 rounded-lg border border-[#F5C542]/20 bg-[#201037]/70 p-4 text-sm naksh-muted-text">Please login before booking. <a className="text-[#FFD36A]" href="/login">Login</a></div> : <p className="mb-5 rounded-lg border border-[#F5C542]/20 bg-[#201037]/70 p-4 text-sm naksh-muted-text">Consultation confirmation will be sent to: {userEmail}</p>}
        <form action={submit} className="grid gap-4">
          ...
          <div className="grid gap-4 md:grid-cols-2"><Field label="Date"><Input name="bookingDate" type="date" required /></Field><Field label="Preferred time slot"><select value={selectedSlot} onChange={(event) => setSelectedSlot(event.target.value)} className="h-10 w-full rounded-md border border-[#F5C542]/20 bg-[#12051f] px-3 text-sm">{slots.map((slot) => <option key={slot.id} value={slot.id}>{slot.consultationType} | {slot.startTime} - {slot.endTime}</option>)}</select></Field></div>
