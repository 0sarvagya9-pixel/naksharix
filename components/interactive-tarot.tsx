...
export function InteractiveTarot() {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-amber-200/20 bg-[radial-gradient(circle_at_20%_10%,rgba(126,72,255,0.24),transparent_24rem),linear-gradient(135deg,rgba(24,9,48,0.92),rgba(8,3,20,0.96))] p-4 sm:p-8">
      <div className="pointer-events-none absolute inset-0 star-field opacity-40" />
      <div className="relative grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-5">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-200">{tr("interactiveTarot")}</p>
            <h2 className="mt-3 font-decorative text-4xl font-black">{tr("shuffleCosmicDeck")}</h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">{tr("tarotIntro")}</p>
          </div>
          <div className="grid gap-3">
            {spreads.map((item) => (
              <button
                ...
                className={`rounded-lg border p-4 text-left transition ${spread.id === item.id ? "border-amber-200 bg-amber-200/10" : "border-amber-200/15 bg-white/[0.04] hover:border-amber-200/45"}`}
              >
                <span className="font-cinzel font-bold">{tr(spreadLabelKey(item.id))}</span>
                <span className="mt-1 block text-xs text-muted-foreground">{item.positions.map((position) => tr(positionKey(position))).join(" | ")}</span>
              </button>
            ))}
          </div>
        ...
        <div className="space-y-8">
          ...
          <Card className="border-amber-200/15 bg-card/70">
            <CardContent className="p-5">
              <p className="font-cinzel text-lg font-bold text-amber-200">{tr("aiInterpretation")}</p>
  ...
  return (
    <div className="relative mx-auto h-48 max-w-md">
      {deckCards.map((_, index) => (
        <motion.div
          key={index}
          className="absolute left-1/2 top-6 h-32 w-20 rounded-lg border border-amber-200/25 bg-[linear-gradient(135deg,rgba(126,72,255,0.88),rgba(245,190,88,0.22))] shadow-[0_0_24px_rgba(126,72,255,0.25)]"
          initial={false}
          animate={{
...
function TarotCardView({ position, card, revealed, index }: { position: string; card?: TarotCard; revealed: boolean; index: number }) {
  ...
  return (
    <motion.div
      className="min-h-48 rounded-lg border border-amber-200/20 bg-white/[0.04] p-4 text-center [transform-style:preserve-3d]"
      animate={{ rotateY: revealed ? 180 : 0, y: revealed ? 0 : index * 8 }}
      transition={{ duration: 0.7, delay: index * 0.12 }}
    >
      <div className="[backface-visibility:hidden]">
        <p className="text-xs uppercase tracking-[0.22em] text-amber-200">{position}</p>
        <div className="mx-auto mt-5 grid h-28 w-20 place-items-center rounded-lg border border-amber-200/25 bg-[linear-gradient(135deg,rgba(126,72,255,0.8),rgba(245,190,88,0.18))]">
      ...
      <div className="-mt-40 [backface-visibility:hidden] [transform:rotateY(180deg)]">
        ...
        <p className="mt-2 text-xs text-muted-foreground">{card?.reversed ? tr("reversed") : tr("upright")}</p>
      </div>
    </motion.div>
  );
}
