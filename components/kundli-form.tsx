        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label={tr("gender")}>
              <select {...form.register("gender")} className="h-10 w-full rounded-xl px-3 text-sm outline-none transition" style={{ background: "rgba(255,252,245,0.90)", border: "1.5px solid rgba(212,160,55,0.45)", color: "#2F2418" }}>
            ...
            <div className="flex items-end">
              <p className="rounded-xl p-3 text-xs leading-5 naksh-muted-text" style={{ background: "rgba(255,252,245,0.70)", border: "1px solid rgba(212,160,55,0.30)" }}><MapPin className="mr-1 inline h-3 w-3" style={{ color: "#D97706" }} />{placeStatus}</p>
            </div>
...
function Meta({ label, value }: { label: string; value?: string }) {
  return (
    <div className="rounded-xl p-3" style={{ background: "rgba(255,252,245,0.70)", border: "1px solid rgba(212,160,55,0.28)" }}>
...
function KundliReport({ result, selectedLanguage, onRegenerate }: { result: KundliResult; selectedLanguage: Locale; onRegenerate: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-2xl p-5 sm:p-6" style={{ background: "linear-gradient(135deg, rgba(255,248,234,0.96) 0%, rgba(255,243,220,0.92) 100%)", border: "1.5px solid rgba(212,160,55,0.50)", boxShadow: "0 12px 48px rgba(180,120,20,0.16)" }}>
        ...
        {!result.saved ? <div className="mt-4 rounded-xl p-4 text-sm" style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(212,160,55,0.30)" }}><p style={{ color: "#2F2418" }}>{tr("loginToSaveReport")}</p><Button className="mt-3" size="sm" asChild><Link href="/login">{tr("login")}</Link></Button></div> : null}
      </section>
      {result.language && result.language !== selectedLanguage ? (
        <div className="rounded-xl p-4 text-sm" style={{ background: "rgba(245,158,11,0.10)", border: "1px solid rgba(212,160,55,0.35)", color: "#2F2418" }}>
      ...
      <div className="rounded-2xl p-5" style={{ background: "linear-gradient(135deg, rgba(255,248,234,0.96), rgba(255,243,220,0.92))", border: "1.5px solid rgba(212,160,55,0.45)" }}>
...
function GeneratingKundli() {
  const { tr } = useLanguage();
  return <div className="mt-4 rounded-xl p-4" style={{ background: "rgba(255,252,245,0.80)", border: "1px solid rgba(212,160,55,0.35)" }}><p className="font-cinzel font-bold" style={{ color: "#2F2418" }}><CalendarDays className="mr-2 inline h-4 w-4" style={{ color: "#D97706" }} />{tr("generatingKundli")}</p><p className="mt-1 text-sm naksh-muted-text">{tr("kundliIntro")}</p></div>;
}

function SummaryCard({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl p-4" style={{ background: "rgba(255,252,245,0.75)", border: "1px solid rgba(212,160,55,0.30)" }}><p className="text-xs uppercase tracking-[0.18em] naksh-muted-text">{label}</p><p className="mt-1 font-cinzel text-lg font-bold" style={{ color: "#2F2418" }}>{value}</p></div>;
}

function InfoPanel({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-xl p-4 text-sm leading-7 naksh-muted-text" style={{ background: "rgba(255,252,245,0.70)", border: "1px solid rgba(212,160,55,0.28)" }}><h3 className="mb-3 font-semibold" style={{ color: "#2F2418" }}>{title}</h3>{children}</section>;
}
