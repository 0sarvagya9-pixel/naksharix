...
export function SavedKundliReportActions({
  report,
  reportId,
  language,
  showView = true,
  ...
  async function downloadPdf() {
    ...
    try {
      const response = await secureFetch("/api/kundli/pdf", {
        ...
        body: JSON.stringify({ report, language, pdfType: isUnlocked ? "PREMIUM" : "FREE" })
      });
      ...
      link.href = url;
      link.download = isUnlocked ? "naksharix-premium-kundli-report.pdf" : "naksharix-free-kundli-report.pdf";
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button size="sm" onClick={downloadPdf} disabled={loading}>
          <Download className="h-4 w-4" />{loading ? tr("preparingPdf") : (isUnlocked ? "Download Premium PDF" : tr("downloadFreePdf"))}
        </Button>
        {isUnlocked ? (
          <span className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-[#D4AF37]/50 bg-[#D4AF37]/10 px-3 text-xs font-bold uppercase tracking-[0.12em] text-[#FFD700]">
            Premium Active
          </span>
        ) : (
